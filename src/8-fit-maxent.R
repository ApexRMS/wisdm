...
if (any(fieldDataSheet$Response == -9998)) {
  ...
  stop(
    "Maxent is a presence-background method; please generate pseudo-absences before continuing. Note: Pseuso-absences can be generated in the '4 - Background Data Generation' Stage of the Pipeline. See 'Background Data Options' for more details."
  )
}
...
if (nrow(validationDataSheet) < 1) {
  validationDataSheet <- addRow(
    validationDataSheet,
    list(SplitData = FALSE, CrossValidate = FALSE)
  )
}
...
if (nrow(out$data$train) / (length(out$inputVars) - 1) < 10) {
  updateRunLog(paste(
    ...
    round(nrow(out$data$train) / (ncol(out$data$train) - 1), digits = 1),
...
out$data$train$predicted <- pred.fct(
  ...
  mod = finalMod,
  modType = modType
) # alternatively should these be pulled from the samplePredictions.csv output by maxent??
...
if ("maxent_StandardResidualPlots.png" %in% tempFiles) {
  modelOutputsSheet$ResidualsPlot <- file.path(
    ssimTempDir,
    paste0(modType, "_StandardResidualPlots.png")
  )
}
if ("maxent_AUCPRPlot.png" %in% tempFiles) {
