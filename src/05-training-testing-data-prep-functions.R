...
testTrainSplit <- function(
  ...
  if (trainProp <= 0 | trainProp > 1) {
    stop("Training Proportion must be a number between 0 and 1 excluding 0")
  }
  ...
  if (sum(as.numeric(response) == 0) == 0 && !is.null(ratioPresAbs)) {
    stop(
      "The ratio of presence to absence cannot be set with only presence data"
    )
  }
  ...
  if (!is.null(factorVars) && length(factorVars) > 0) {
    for (i in seq_along(factorVars)) {
      ...
      if (any(factor.table < 10)) {
        updateRunLog(paste0(
          "\nSome levels for the categorical predictor ",
          factorVars[i],
  ...
  if (length(response) < 100) {
    stop(
      "A test training split is not advisable for less than 100 observations.  Consider-cross validation as an alternative."
    )
  }
  ...
  if (all(na.omit(response) %in% 0:1) & any(table(response) < 10)) {
    stop(
      "Use of a test training split is not recommended when the dataset contains less than 10 presence or absence points"
    )
  }
  ## End warnings section ##

  temp <- if (!is.null(ratioPresAbs)) {
    (sum(response >= 1) / sum(response == 0) == ratioPresAbs)
  }
  if (is.null(temp)) {
    temp <- FALSE
  }
  ...
  if (is.null(ratioPresAbs) | temp) {
    ...
    for (i in sort(as.numeric(unique(response)))) {
      TrainSplit <- c(
        TrainSplit,
        sample(
          which(response == i, arr.ind = TRUE),
          size = round(sum(response == i) * trainProp)
        )
      )
    }
    ...
    TestSplit <- which(
      !(seq(1:length(response))) %in% TrainSplit,
      arr.ind = TRUE
    )
    ...
    # now considering if there is a desired ratio of presence to absence points
    ...
    if (sum(response >= 1) / sum(response == 0) >= ratioPresAbs) {
      ...
      EachToRmv <- round(
        TotToRmv * table(response[response != 0]) / sum(response != 0)
      )
      ByCount <- split(
        cbind(which(response != 0, arr.ind = TRUE)),
        f = response[response != 0]
      )

      # sampling function ----
      sam <- function(x, size) {
        if (length(x) == 1 & size == 1) {
          return(x)
        } else {
          sample(x = x, size = size)
        }
      }
      ...
      for (i in sort(as.numeric(unique(respKeep)))) {
        TrainSplit <- c(
          TrainSplit,
          sample(
            names(respKeep[respKeep == i]),
            size = round(sum(respKeep == i) * trainProp)
          )
        )
      }
      TrainSplit <- as.numeric(TrainSplit)
      # Take everything not in the training set or in the remove list for the test set
      TestSplit <- seq(from = 1, to = length(response))[
        -c(c(TrainSplit, RmvIndx))
      ]
    ...
    if (sum(response >= 1) / sum(response == 0) < ratioPresAbs) {
      ...
      if (
        (sum(response == 0) - sum(response >= 1) / ratioPresAbs) /
          sum(response == 0) >
          0.5
      ) {
        updateRunLog(
      ...
      TrainSplit <- c(
        TrainSplit,
        sample(
          which(response == 0, arr.ind = TRUE),
          size = round(sum(response >= 1) * (1 / ratioPresAbs) * trainProp)
        )
      )
      ...
      temp <- sample(
        which(
          TestSplit %in% which(response == 0, arr.ind = TRUE),
          arr.ind = TRUE
        ),
        size = round(
          sum(TestSplit %in% which(response == 0, arr.ind = TRUE)) -
...
crossValidationSplit <- function(
  ...
  options(warn = 1)
  if (nFolds <= 1 | nFolds %% 1 != 0) {
    stop("Number of folds must be an integer greater than 1")
  }
    ...
    for (i in seq_along(factorVars)) {
      ...
      if (any(factor.table < nFolds)) {
        updateRunLog(paste0(
          ...
          factorVars[i],
          " have fewer observations than the number of CV folds (",
          nFolds,
          "). \n",
          "This increases the likelihood that a CV training fold will not contain all factor levels,",
          " which may cause prediction failures during cross-validation.\n",
          "Consider removing or reclassifying rare levels, or reducing the number of CV folds before continuing.\n"
        ))
        factor.table <- as.data.frame(factor.table)
        colnames(factor.table) <- c("Factor Level", "Observation Count")
        updateRunLog(paste("\n", factorVars[i], "\n"))
        updateRunLog(pander::pandoc.table.return(
          factor.table,
          style = "rmarkdown"
  ...
  if (any(dat$UseInModelEvaluation == TRUE)) {
    ...
    split.mask <- index <- seq(1:nrow(dat))
  }

  if (stratify & !spatSplit) {
    # if stratify = T and spatial split = F
    ...
    for (i in 1:length(names(table(response)))) {
      ...
      dat$ModelSelectionSplit[index.i] <- c(
        rep(seq(1:nFolds), each = floor(length(index.i) / nFolds)),
    ...
    # if stratify = F and spatial split = F

    index <- index[order(runif(length(index)))]
    dat$ModelSelectionSplit[index] <- c(
      rep(seq(1:nFolds), each = floor(length(index) / nFolds)),
      sample(seq(1:nFolds), size = length(index) %% nFolds, replace = FALSE)
    )
  }

  if (
    all(na.omit(response) %in% 0:1) &
      any(table(dat$Response, dat$ModelSelectionSplit) < 2)
  ) {
    stop(
      "Some combinations of the response and data split are nearly empty, please use a stratified nonspatial split"
    )
  }
