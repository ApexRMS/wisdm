...
detect_system_memory <- function() {
  ...
  if (sys == "windows") {
    ...
    if (
      nzchar(paste(j, collapse = "")) &&
        requireNamespace("jsonlite", quietly = TRUE)
    ) {
      dat <- tryCatch(
        jsonlite::fromJSON(paste(j, collapse = "")),
  ...
  if (sys == "darwin") {
    ...
    total_b <- suppressWarnings(as.numeric(system(
      "sysctl -n hw.memsize",
      intern = TRUE
    )))
    # Available (approx): free + inactive pages * page size
    pagesize <- suppressWarnings(as.numeric(system(
      "sysctl -n hw.pagesize",
      intern = TRUE
    )))
    vm <- tryCatch(system("vm_stat", intern = TRUE), error = function(e) {
      character()
    })
    ...
    avail_b <- if (
      is.finite(pagesize) && is.finite(free_p) && is.finite(inactive_p)
    ) {
      (free_p + inactive_p) * pagesize
    } else {
      NA_real_
    }
...
setup_session <- function(
  ...
  if (is.null(memmax_gb)) {
    if (
      is.numeric(total_ram_gb) && is.finite(total_ram_gb) && total_ram_gb > 0
    ) {
  ...
  if (desync_max_sec > 0) {
    Sys.sleep(stats::runif(1, 0, desync_max_sec))
  }
...
non_NA_extent <- function(x, block_rows = 2048L) {
  ...
  for (rr in top:bottom) {
    ...
    if (length(nz)) {
      left <- min(left, nz[1])
      right <- max(right, tail(nz, 1))
    }
  }
  readStop(x)
  ext(
    xFromCol(x, left) - 0.5 * rs[1],
    xFromCol(x, right) + 0.5 * rs[1],
    yFromRow(x, bottom) - 0.5 * rs[2],
    yFromRow(x, top) + 0.5 * rs[2]
  )
}
...
build_cov_stack <- function(
  cov_data, # dataframe of CovariatesID, RasterFilePath
  ...
  if (any(!file.exists(paths))) {
    ...
    stop(
      "Missing rasters in Covariate Data: ",
      paste(missingTifs, collapse = ", ")
    )
  }
...
predict_block_int <- function(
  ...
  if (length(factor_levels)) {
    for (j in names(factor_levels)) {
      if (j %in% names(data)) {
        lv <- factor_levels[[j]]
        data[[j]] <- if (is.null(lv)) {
          factor(data[[j]])
        } else {
          factor(data[[j]], levels = lv)
        }
      }
    }
  }
...
prepare_mess <- function(train.dat, var_names) {
  ...
  for (nm in names(td)) {
    if (is.factor(td[[nm]])) td[[nm]] <- as.numeric(td[[nm]])
  }
...
CalcMESS <- function(rast_block, prep) {
  ...
  for (nm in names(x)) {
    if (is.factor(x[[nm]])) x[[nm]] <- as.numeric(x[[nm]])
  }
  x_mat <- as.matrix(x)
  ...
  if (any(inside)) {
    rank_mat <- percent_rank_distance(
      x_mat[inside, , drop = FALSE],
      prep$sorted_cols,
      prep$n
    )
...
range_proximity_pct <- function(x_mat, mins, maxs) {
  stopifnot(
    is.matrix(x_mat),
    length(mins) == ncol(x_mat),
    length(maxs) == ncol(x_mat)
  )
