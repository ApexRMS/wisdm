...
covariateCorrelationSheet <- datasheet(
  myScenario,
...
if (nrow(covariateSelectionSheet) < 1) {
  covariateSelectionSheet <- addRow(
    covariateSelectionSheet,
    list(SelectionMethod = "Interactive (Correlation Viewer)")
  )
}

if (is.na(covariateSelectionSheet$SelectionMethod)) {
  covariateSelectionSheet$SelectionMethod <- "Interactive (Correlation Viewer)"
}
...
siteData <- select(
  siteDataWide,
  -c(SiteID, X, Y, UseInModelEvaluation, ModelSelectionSplit, Weight)
) #
...
if (sum(covariatesSheet$IsCategorical, na.rm = T) > 0) {
  factorVars <- covariatesSheet$CovariateName[which(
    covariatesSheet$IsCategorical == T &
      covariatesSheet$CovariateName %in% names(siteData)
  )]
  if (length(factorVars) > 0) {
    badFactors <- NULL
    for (i in 1:length(factorVars)) {
      factor.table <- table(siteData[, factorVars[i]])
      if (length(factor.table) < 2) {
        badFactors <- c(badFactors, factorVars[i])
      }
    }
    if (length(badFactors) > 0) {
      ...
      updateRunLog(paste0(
        ...
        "because they had only one level: ",
        paste(badFactors, collapse = ","),
        "\n"
      ))
    }
  } else {
...
if (
  covariateSelectionSheet$SelectionMethod ==
    "Automatic (Variance Inflation Factor)"
) {
  ## Covariate selection
  if (is.na(covariateSelectionSheet$CorrelationThreshold)) {
    covariateSelectionSheet$CorrelationThreshold <- 0.7
  }
  if (is.na(covariateSelectionSheet$VIFThreshold)) {
    covariateSelectionSheet$VIFThreshold <- 10
  }
  ...
  if (length(retainedCovariatesSheet$CovariatesID) > 0) {
    colin <- vifstep(
      x = covData,
      th = covariateSelectionSheet$VIFThreshold,
      keep = as.character(retainedCovariatesSheet$CovariatesID)
    )
  } else {
  ...
  updateRunLog(
    ...
    " of the ",
    length(allCovs),
    " input variables were removed from the covariate list due to collinearity:\n\n",
  ...
  corVals <- unique(corMat[
    corMat > covariateSelectionSheet$CorrelationThreshold
  ])
...
if (
  ...
  ## Covariate selection
  if (is.na(covariateSelectionSheet$DisplayHighestCorrelations)) {
    covariateSelectionSheet$DisplayHighestCorrelations <- TRUE
  }
  if (is.na(covariateSelectionSheet$CorrelationThreshold)) {
    covariateSelectionSheet$CorrelationThreshold <- 0.7
  }
  if (is.na(covariateSelectionSheet$NumberOfPlots)) {
    covariateSelectionSheet$NumberOfPlots <- 5
  }
  ...
  if (is.null(browser.path)) {
    runApp(
      appDir = file.path(packageDir, "06-covariate-correlation-app.R"),
      launch.browser = TRUE
    )
  } else {
    runApp(
      ...
      launch.browser = function(shinyurl) {
        system(
          paste0("\"", browser.path, "\" --app=", shinyurl, " -incognito"),
          wait = F
        )
      }
    )
  ...
  covariateCorrelationSheet <- addRow(
    ...
    data.frame(
      InitialMatrix = file.path(
        ssimTempDir,
        "InitialCovariateCorrelationMatrix.png"
      ),
      SelectedMatrix = file.path(
        ssimTempDir,
...
selectedCovariates <- unique(c(
  as.character(retainedCovariatesSheet$CovariatesID),
  selectedCovariates
))
