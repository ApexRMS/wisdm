...
if (all(fieldDataSheet$Response == 1) | all(fieldDataSheet$Response == 0)) {
  stop(
    "GAM is a presence-absences method; please ensure that the Field Data includes both presence and absence (or pseudo-absence) data before continuing."
  )
}
...
capture.output(
  cat("Generalized Additive Model Results"),
...
txt0 <- paste(
  ...
  "\n\t model family:  ",
  out$modelFamily,
  "\n\t Shrinkage smoothers used:  ",
  ifelse(GAMSheet$AllowShrinkageSmoothers, "yes", "no"),
  "\n\n\n",
  "Results:\n\t "
)
