...
fitModel <- function(
  ...
  # define columns in training data not used for model fitting
  nonCovariateCols <- c(
    "SiteID",
    "X",
    "Y",
  ...
  if (out$modType == "rf") {
    ...
    if (is.na(out$modOptions$NumberOfVariablesSampled)) {
      ...
      mtry = out$modOptions$NumberOfVariablesSampled
    }
    if (is.na(out$modOptions$MaximumNodes)) {
      maxnodes = NULL
    } else {
      maxnodes = out$modOptions$MaximumNodes
    }
...
read.maxent <- function(lambdas) {
  ...
  Raw.coef <- Quad.coef <- Prod.coef <- Fwd.Hinge <- Rev.Hinge <- Thresh.val <- Raw.mult <- Quad.mult <- Prod.mult <- FH.mult <- FH.cnst <- Rev.mult <- Rev.cnst <- Thresh.cnst <- NULL

  if (any(fctType == "raw")) {
    "Raw.coef" <- lambdas[fctType == "raw", ]
    Raw.mult <- c(
      -sum(Raw.coef[, 2] * Raw.coef[, 3] / (Raw.coef[, 4] - Raw.coef[, 3])),
      Raw.coef[, 2] / (Raw.coef[, 4] - Raw.coef[, 3])
    )
  ...
  if (any(fctType == "quadratic")) {
    ...
    Quad.mult <- c(
      -sum(Quad.coef[, 2] * Quad.coef[, 3] / (Quad.coef[, 4] - Quad.coef[, 3])),
      Quad.coef[, 2] / (Quad.coef[, 4] - Quad.coef[, 3])
    )
  }
  if (any(fctType == "product")) {
    ...
    Prod.mult <- c(
      -sum(Prod.coef[, 2] * Prod.coef[, 3] / (Prod.coef[, 4] - Prod.coef[, 3])),
      Prod.coef[, 2] / (Prod.coef[, 4] - Prod.coef[, 3])
    )
  }
  ...
  if (any(fctType == "reverse.hinge")) {
    ...
    Rev.cnst <- Rev.Hinge[, 2] *
      Rev.Hinge[, 4] /
      (Rev.Hinge[, 4] - Rev.Hinge[, 3])
  }
...
est.lr <- function(dat, out) {
  # define non-covariate columns in dat
  nonCovariateCols <- c(
    "SiteID",
    "X",
    "Y",
...
cv.fct <- function(
  ...
  if (family == "binomial" | family == "bernoulli") {
    full.resid.deviance <- calc.deviance(
      obs,
      preds,
  ...
  for (i in 1:nk) {
    ...
    if (out$modType == "maxent") {
      ...
      if (!file.exists(file.path(out$tempDir, "CVsplits"))) {
        dir.create(file.path(out$tempDir, "CVsplits"))
      }
      ...
      data[model.mask, ] %>%
        ...
        write.csv(
          file.path(out$tempDir, "CVsplits", "training-swd.csv"),
          row.names = F
        )
      ...
      data[model.mask, ] %>%
        ...
        relocate(Species, .before = X) %>%
        write.csv(
          file.path(out$tempDir, "CVsplits", "background-swd.csv"),
          row.names = F
        )
    }

    # fit new model
    cv.final.mod <- fitModel(
      dat = data[model.mask, ],
      out = out,
  ...
  subset.test <- list(
    test.scores = subset.test,
    subset.test.mean = subset.test.mean,
  ...
  subset.deviance <- list(
    subset.deviances = subset.resid.deviance,
    subset.deviance.mean = subset.deviance.mean,
    subset.deviance.se = subset.deviance.se
  )

  cv.list <- list(
    full.resid.deviance = full.resid.deviance,
    full.test = full.test,
    full.calib = full.calib,
    pooled.deviance = cv.resid.deviance,
    pooled.test = cv.test,
    pooled.calib = cv.calib,
    subset.deviance = subset.deviance,
    subset.test = subset.test,
    subset.calib = subset.calib,
    cor.mat = cor.mat,
    thresh = thresh,
    cv.train.data = data
  ) # , resp.curves=resp.curves,
...
roc <- function(
  ...
  if (length(obs) != length(preds)) {
    stop(
      "ROC function failed - observed and predicted response must be equal lengths"
    )
  }
  ...
  rnk <- rank(xy)
  wilc <- ((n.x * n.y) + ((n.x * (n.x + 1)) / 2) - sum(rnk[1:n.x])) /
    (n.x * n.y)
  return(round(wilc, 4))
}
...
calibration <- function(
  ...
  if (pred.range > 1.2 & family == "binomial") {
    updateRunLog(paste0(
      "Range of response variable is ",
      round(pred.range, 2),
      ". Check family specification."
    ))
  }
  if (family == "binomial") {
    pred <- preds + 1e-005
...
permute.predict <- function(
  ...
  for (j in 1:5) {
    # do the permutation 5 times to remove some of the random chance component
...
makeModelEvalPlots <- function(out = out) {
  # previous function name: make.auc.plot.jpg

  standResidualFile <- file.path(
    out$tempDir,
    paste0(out$modType, "_StandardResidualPlots.png")
  )
  variableImportanceFile <- file.path(
    out$tempDir,
    paste0(out$modType, "_VariableImportance.png")
  )
  confusionMatrixFile <- file.path(
    out$tempDir,
    paste0(out$modType, "_ConfusionMatrix.png")
  )
  residualSmoothFile <- file.path(
    out$tempDir,
    paste0(out$modType, "_ResidualSmoothPlot.png")
  )
  residualSmoothFctFile <- file.path(
    out$tempDir,
    paste0(out$modType, "_ResidualSmoothFunction.rds")
  )
  ...
  calibrationFile <- file.path(
    out$tempDir,
    paste0(out$modType, "_CalibrationPlot.png")
  )
  residualPlotFile <- file.path(
    out$tempDir,
    paste0(out$modType, "_PoissonResidualPlots.png")
  )
  ...
  if (out$modelFamily != "poisson") {
    ...
    out$trainThresh <- as.numeric(optimal.thresholds(
      data.frame(
        ID = 1:length(out$data$train$Response),
    ...
    if (!is.null(out$data$test)) {
      out$testThresh <- as.numeric(optimal.thresholds(
        data.frame(
          ID = 1:length(out$data$test$Response),
  ...
  if (out$validationOptions$CrossValidate) {
    for (i in 1:out$validationOptions$NumberOfFolds) {
      Stats[[i]] <- calcStat(
        x = out$cvResults$cv.train.data[
          out$cvResults$cv.train.data$ModelSelectionSplit == i,
        ],
        family = out$modelFamily,
  ...
  Stats$train <- calcStat(
    x = out$data$train,
    family = out$modelFamily,
    thresh = out$trainThresh,
    has.split = hasSplit
  )
  ...
  if (splitType %in% c("none", "cv")) {
    ...
    residualSmoothFct <- resid.image(
      dev.contrib = dev.contrib,
      dat = out$data$train,
  ...
  if (out$modelFamily %in% c("binomial", "bernoulli")) {
    ...
    if (splitType == "none") {
      legend(
        x = .8,
        y = .15,
        paste("AUC=", round(Stats$train$auc.fit, digits = 3), sep = "")
      )
    } else {
      if (splitType == "train/test") {
        ...
        legend(
          ...
          c(
            paste(
              "Training Split (AUC=",
              round(Stats$train$auc.fit, digits = 3),
              ")",
              sep = ""
            ),
            paste(
              "Testing Split  (AUC=",
              round(Stats$test$auc.fit, digits = 3),
              ")",
              sep = ""
            )
          ),
      ...
      if (splitType %in% c("cv", "cv/train/test")) {
        ...
        ROC.list <- list(
          predictions = lapply(lst, function(lst) {
            lst$auc.data$pred
          }),
          labels = lapply(lst, function(lst) {
            lst$auc.data$pres.abs
          })
        )
        ...
        plot(
          ...
          main = "ROC Plot for Cross-Validation",
          cex.main = 2,
          cex.axis = 1.4,
          cex.lab = 1.5
        )
        ...
        text(
          x = (.96 - Stats$train$Specf),
          y = Stats$train$Sens + .03,
          label = round(Stats$train$thresh, digits = 2)
        )
        ...
        legend(
          ...
          c(
            ...
            paste(
              ...
              round(
                mean(unlist(lapply(lst, function(lst) {
                  lst$auc.fit
                }))),
                digits = 3
              ),
    ...
    if (splitType %in% c("cv", "cv/train/test")) {
      ...
      for (n in 1:(length(lst))) {
        tmp <- pr.curve(
          scores.class0 = lst[[n]]$auc.data$pred,
          weights.class0 = lst[[n]]$auc.data$pres.abs,
          curve = T
        )
      ...
      for (i in 1:length(aucpr.lst)) {
        tmp <- aucpr.lst[[i]]$curve %>% summarise_all(.funs = mean)
        cv.df <- bind_rows(cv.df, tmp)
      }

      # Add training data #
      pr <- pr.curve(
        scores.class0 = Stats$train$auc.data$pred,
      ...
      pl <- pl +
        suppressWarnings(geom_line(
          data = as.data.frame(spline(pr$curve$recall, pr$curve$precision)),
        ...
        scale_color_manual(
          labels = c(
            paste0("Training Split (AUC=", round(Stats$train$auc.pr, 3), ")"),
            paste0(
              "Cross Validation Mean (AUC=",
              mean(round(mean.vec, digits = 3)),
              ")",
              sep = ''
            )
          ),
    ...
    if (out$pseudoAbs) {
      ...
      p.plt <- try(
        pocplot(
          ...
          a$pred[a$pres.abs == 0],
          title = paste0(
            "Presence Only Calibration Plot for \n",
            switch(
              splitType,
              "none" = "Training Data",
  ...
  if (out$modelFamily %in% c("poisson")) {
    ...
    plot(
      log(Stats$train$auc.data$pred[Stats$train$auc.data$pred != 0]),
      (Stats$train$auc.data$pres.abs[Stats$train$auc.data$pred != 0] -
        Stats$train$auc.data$pred[Stats$train$auc.data$pred != 0]),
      xlab = "Predicted Values (log scale)",
      ylab = "Residuals",
    ...
    if (out$modType != "rf") {
      ...
      plot(
        log(Stats$train$auc.data$pred[Stats$train$auc.data$pred != 0]),
        sqrt(
          (abs(residuals(out$finalMod, type = "deviance")[
            Stats$train$auc.data$pred != 0
          ]))
        ),
        xlab = "Predicted Values (log Scale)",
        ylab = yl
  ...
  if (splitType == "cv/train/test") {
    ...
    capture.stats(
      Stats[1:(out$validationOptions$NumberOfFolds)],
      file.name = file.path(out$tempDir, paste0(out$modType, "_output.txt")),
      label = "cv",
      family = out$modelFamily,
      opt.methods = out$modOptions$thresholdOptimization,
      out
...
calcStat <- function(
  ...
  if (has.split & !all(x$UseInModelEvaluation)) {
    # we're in the train split here for pseudoAbs
    dev.vect <- vector()
    null.dev <- vector()
    for (i in 1:(length(unique(x$ModelSelectionSplit)) - 1)) {
      dev.vect[i] <- calc.deviance(
        c(
          rep(0, times = sum(x$ModelSelectionSplit == i)),
          x$Response[x$Response == 1]
        ),
        c(x$predicted[x$ModelSelectionSplit == i], x$predicted[x$Response == 1])
      )
      null.dev[i] <- calc.deviance(
        c(
          rep(0, times = sum(x$ModelSelectionSplit == i)),
          x$Response[x$Response == 1]
        ),
        rep(
          mean(c(
            rep(1, times = sum(x$Response == 1)),
            rep(0, times = sum(x$UseInModelEvaluation == i))
          )),
  ...
  } else if (has.split & all(x$UseInModelEvaluation)) {
    # in the test split for pseudoAbs -- deviance shouldn't be calculated because calibration is wrong
    null.dev = NA
    dev.fit = NA
  } else {
    null.dev <- calc.deviance(
      auc.data$pres.abs,
      rep(p.bar, times = length(auc.data$pres.abs)),
      x$Weight,
      family = family
    ) #*nrow(x$dat)
    if (is.nan(null.dev)) {
    ...
    dev.fit <- calc.deviance(
      auc.data$pres.abs,
      auc.data$pred,
      x$Weight,
      family = family
    ) #*nrow(x$dat) Elith does not include this it might cause a weighting issue when averaging I'm not sure if I should include it
    if (is.nan(dev.fit)) {
      dev.fit <- NA
    }
  }
  ...
  if (family %in% c("binomial", "bernoulli")) {
    ...
    return(list(
      ...
      Kappa = KAPPA,
      Tss = TSS,
      calibration.stats = calibration.stats,
      thresh = thresh
    ))
  }
...
VariableImportance <- function(
  ...
  # list of auc.fit values output by calcStat function
  ...
  if (out$validationOptions$CrossValidate) {
    cor.mat[, (length(cnames) + 1):(ncol(cor.mat))] <- -t(apply(
      out$cvResults$cor.mat,
      1,
      "-",
      as.vector(unlist(auc)[1:out$validationOptions$NumberOfFolds])
    ))
  ...
  variable.importance.csv <- file.path(
    out$tempDir,
    paste0(out$modType, "_VariableImportance.csv")
  )
  write.table(
    cbind(predictor = out$inputVars, cor.mat),
  ...
  if (!out$validationOptions$SplitData & !out$validationOptions$CrossValidate) {
    plot(
      x = c(min(0, min(xright[, "train"])), (max(xright[, "train"]) + 0.1)),
  ...
  if (!out$validationOptions$SplitData & out$validationOptions$CrossValidate) {
    plot(
      x = c(min(0, min(xright)), (max(xright) + 0.1)),
      y = c(-0.5, (length(out$inputVars) + 0.5)),
      type = "n",
      xlab = "Importance",
      ylab = "",
    ...
    legend(
      ...
      pt.cex = c(3, 3.5),
      pt.lwd = c(2, 3),
      pt.bg = c("lightblue", "darkslateblue"),
  ...
  if (out$validationOptions$SplitData & out$validationOptions$CrossValidate) {
    ...
    legend(
      x = "bottomright",
      legend = c("CV", "Train", "Test"),
      pch = c(22, 8, 5),
      pt.cex = c(3.5, 2.75, 2.5),
      pt.lwd = c(2, 3, 3),
      pt.bg = c("lightblue", "darkslateblue", "royalblue"),
      col = c("black", "darkslateblue", "royalblue"),
      cex = 2.5
    )
  }

  Offset <- ifelse(
    !out$validationOptions$SplitData & !out$validationOptions$CrossValidate,
    0.25,
    0
  )
  ...
  axis(
    ...
    at = seq(
      from = 0,
      to = length(out$inputVars),
      length = length(out$inputVars)
    ) +
      Offset,
    labels = ylabs,
    las = 2,
...
confusion.matrix <- function(
  ...
  if (!out$validationOptions$SplitData & !out$validationOptions$CrossValidate) {
    ...
    if (out$validationOptions$CrossValidate) {
      a <- lapply(
        Stats[-which(names(Stats) %in% c("train", "test"))],
        function(lst) {
          lst$Cmx
        }
      )
  ...
  for (i in length(Stats):1) {
    image(
      ...
      matrix(
        data = c(
          100 * Stats[[i]]$Cmx[2] / sum(Stats[[i]]$Cmx[1:2]),
    ...
    mtext(
      paste(
        ...
        signif(Stats[[i]]$Pcc, digits = 3),
        "\nSensitivity                      : ",
        signif(Stats[[i]]$Sens, digits = 3),
        "\nSpecificity                      : ",
        signif(Stats[[i]]$Specf, digits = 3),
        "\nTrue Skills Stat              : ",
        signif(Stats[[i]]$Tss, digits = 3),
        "\nCohen's Kappa              : ",
        signif(Stats[[i]]$Kappa, digits = 3),
        sep = ""
      ),
      side = 1,
      line = 13,
  ...
  image(
    1,
    seq(from = zlim[1], to = zlim[2], length = 50),
    matrix(
      data = seq(from = zlim[1], to = zlim[2], length = 50),
      ncol = 50,
      nrow = 1
    ),
...
resid.image <- function(dev.contrib, dat, file.name, label, create.image = T) {
  ...
  if (length(dat$predicted) > 2000) {
    samp <- seq(1:length(dat$predicted))[order(runif(length(dat$predicted)))][
      1:2000
    ]
  ...
  if (label != "eval") {
    z <- sign(dat$Response - dat$predicted) * abs(dev.contrib)
  } else {
    z <- dat$Response - dat$predicted
  }
...
beachcolours <- function(
  ...
  if (nbeach > 0) {
    colours <- c(
      colours,
      rev(rainbow(nbeach, start = 3 / 12, end = 5 / 12, s = s))
    )
  }
  if (nland > 0) {
...
TestTrainRocPlot <- function(
  ...
  if (is.data.frame(dat) == FALSE) {
    if (is.matrix(dat) == TRUE) {
      dat <- as.data.frame(dat)
    } else {
      stop("Test/Train ROC failed - 'dat' must be either data frame or matrix")
    }
  }
  ...
  if (length(obs[obs == 0]) == 0) {
    stop(
      "Test/Train ROC failed - no observed absences in dataset, therefore specificity does not",
  ...
  if (length(obs[obs == 1]) == 0) {
    stop(
      "Test/Train ROC failed - no observed presences in dataset, therefore sensitivity does not",
  ...
  if (length(smoothing) != 1) {
    stop(
      "Test/Train ROC failed - 'smoothing' must be a single number greater than or equal to 1"
    )
  } else {
  ...
  if (sum(is.na(dat)) > 0) {
    if (na.rm == TRUE) {
      ...
      updateRunLog(paste0(
        "Warning - Test/Train ROC: ",
        length(NA.rows[NA.rows > 0]),
        " rows of data were ignored due to NA values"
      ))
      dat <- dat[NA.rows == 0, ]
    } else {
      return(NA)
  ...
  if (obs.prev < 0 || obs.prev > 1) {
    stop(
      "Test/Train ROC failed - 'obs.prev' must be a number between zero and one"
    )
  }

  if (obs.prev == 0) {
    updateRunLog(
      "Warning - Test/Train ROC: Because the observed prevalence was zero, results may be strange"
    )
  }

  if (obs.prev == 1) {
    updateRunLog(
      "Warning - Test/Train ROC: Because the observed prevalence was one, results may be strange"
    )
  }

  if (min(which.model) < 1 || sum(round(which.model) != which.model) != 0) {
    stop(
      "Test/Train ROC failed - values in 'which.model' must be positive integers"
    )
  }

  if (max(which.model) > N.models) {
    stop(
      "Test/Train ROC failed - values in 'which.model' must not be greater than number of models in 'dat'!"
    )
  }
  ...
  if (
    N.models != length(model.names) &&
      (length(which.model) != 1 || length(model.names) != 1)
  ) {
    stop(
      "Test/Train ROC failed - If 'model.names' is specified it must either be a single name, or a vector",
  ...
  if (length(legend.text) != N.models) {
    stop(
      "Test/Train ROC failed - 'opt.legend.text' must be of same length as 'opt.methods'"
    )
  }
  ...
  if (is.logical(opt.thresholds) == TRUE) {
    if (opt.thresholds == TRUE) {
      ...
      if ("ReqSens" %in% opt.methods) {
        if (missing(req.sens)) {
          updateRunLog(
            "Warning - Test/Train ROC: required sensetivity defaults to 0.85"
          )
          req.sens <- 0.85
        }
      }
      ...
      if ("ReqSpec" %in% opt.methods) {
        if (missing(req.spec)) {
          updateRunLog(
            "Warning - Test/Train ROC: required specificity defaults to 0.85"
          )
          req.spec <- 0.85
        }
      }
      ...
      if (is.null(pch) == TRUE) {
        pch <- c(1, 5, 2, 16, 15, 17, 8, 6, 9, 12, 4, 7)[match(
          opt.methods,
          POSSIBLE.meth
        )]
      } else {
  ...
  if (is.logical(opt.thresholds) == FALSE) {
    if (!is.numeric(opt.thresholds)) {
      stop(
        "Test/Train ROC failed - 'opt.thresholds' must be 'TRUE', 'FALSE', or numeric"
      )
    }
    ...
    if (max(opt.thresholds) > 1) {
      if (N.thr == 1 && round(opt.thresholds) == opt.thresholds) {
        ...
        stop(
          "Test/Train ROC failed - non-interger, non-logical 'opt.thresholds' greater than 1"
        )
      }
    }
    ...
    if (length(opt.legend.text) != N.opt.thresh) {
      stop(
        "Test/Train ROC failed - length of 'opt.legend.text' does not match number of specified thresholds"
      )
    }
    ...
    if (length(pch) != N.opt.thresh) {
      stop(
        "Test/Train ROC failed - length of 'pch' does not match number of specified thresholds"
      )
    }
  ...
  for (d in 1:N.dat) {
    Model.dat <- roc.plot.calculate(
      DATA = dat,
      threshold = threshold,
      which.model = d
    )
    lines(
      x = (1 - Model.dat$specificity),
      y = Model.dat$sensitivity,
      lty = line.type[d],
      lwd = lwd,
      col = colors[d]
    )
    if (max(mark) != 0) {
      Mark.dat <- roc.plot.calculate(
        DATA = dat,
        threshold = mark[, d],
        which.model = d
      )
      ...
      Mark.pretty.char[nchar(Mark.pretty.char) == 3] <- paste(
        Mark.pretty.char[nchar(Mark.pretty.char) == 3],
        "0",
        sep = ""
      )
      if (add.roc == FALSE) {
        points(
          x = (1 - Mark.dat$specificity),
          y = Mark.dat$sensitivity,
          cex = 2,
          pch = pch,
          col = mark.colors[d]
        )
        ...
        if (mark.numbers == TRUE) {
          text(
            x = (1 - Mark.dat$specificity),
            y = Mark.dat$sensitivity - 0.03,
            labels = Mark.pretty.char,
            pos = 4,
            col = mark.colors[d]
          )
        }
    ...
    if (cost.line == TRUE) {
      ...
      abline(
        a = Mark.dat$sensitivity[tag] - ((1 - Mark.dat$specificity[tag]) * sl),
        b = sl,
        col = mark.colors[d],
        lty = 3
      )
  ...
  if (add.legend == TRUE) {
    ...
    if (find.auc == TRUE) {
      ...
      AUC.pretty.char[nchar(AUC.pretty.char) == 3] <- paste(
        AUC.pretty.char[nchar(AUC.pretty.char) == 3],
        "0",
        sep = ""
      )
...
capture.stats <- function(
  ...
  capture.output(cat(
    ...
    if (!out$hasSplit | (out$hasSplit & label == "Training")) {
      paste(
        ...
        if (out$hasSplit & label == "Training") {
          " (Averaged over background splits)"
        },
        if (label == "Cross validation") {
          paste(
            " (sd ",
  ...
  if (family %in% c("binomial", "bernoulli")) {
    capture.output(
      ...
      if (label %in% c("Training", "Final evaluation")) {
        print.table(Stats.lst[[1]]$Cmx)
      } else {
        a <- lapply(Stats.lst, function(lst) {
          lst$Cmx
        })
        cmx <- a[[1]]
        for (i in 2:length(a)) {
  ...
  if (!out$pseudoAbs) {
    capture.output(cat(
      ...
      "\n\t Intercept (general calibration)                            : ",
      mean(unlist(lapply(Stats.lst, function(lst) {
        lst$calibration.stats[1]
      }))),
      if (label == "Cross validation") {
        paste(
          " (sd ",
      ...
      "\n\t Slope   (direction and variation in fit)                   : ",
      mean(unlist(lapply(Stats.lst, function(lst) {
        lst$calibration.stats[2]
      }))),
      if (label == "Cross validation") {
        paste(
          " (sd ",
      ...
      "\n\t Testa0b1 (overall reliability of predictors)               : ",
      mean(unlist(lapply(Stats.lst, function(lst) {
        lst$calibration.stats[3]
      }))),
      if (label == "Cross validation") {
        paste(
          " (sd ",
      ...
      "\n\t Testa0|b1(incorrect calibration given correct refinement)  : ",
      mean(unlist(lapply(Stats.lst, function(lst) {
        lst$calibration.stats[4]
      }))),
      if (label == "Cross validation") {
        paste(
          " (sd ",
      ...
      "\n\t Testb1|a (refinement given correct calibration)            : ",
      mean(unlist(lapply(Stats.lst, function(lst) {
        lst$calibration.stats[5]
      }))),
      if (label == "Cross validation") {
        paste(
          " (sd ",
...
response.curves <- function(out) {
  ...
  if (k == 1) {
    png(
      file.path(out$tempDir, paste0(out$modType, "_ResponseCurves.png")),
      width = 2000,
      height = 2000,
      pointsize = 20
    )
  ...
  for (i in sort(match(out$finalVars, names(dat)))) {
    if (k == 2) {
      png(
        filename = file.path(
          out$tempDir,
          paste0(out$modType, "_ResponseCurves"),
          paste0(names(dat)[i], ".png", sep = "")
        ),
        width = 2000,
        height = 2000,
        pointsize = 20
    ...
    if (k == 2) {
      mtext("Predicted Value", side = 2, line = 5, cex = 3.5)
    }
    axis(1, labels = FALSE)
    axis(2, labels = FALSE)
