...
modType <- modelsSheet$ModelType[
  modelsSheet$ModelName == tuningModelSheet$Model
]
if (modType == "rf") {
...
outputHyperparameterTuningSheet <- datasheet(
  myScenario,
  "wisdm_OutputHyperparameterTuning",
  optional = T,
  returnInvisible = T,
  empty = T,
  lookupsAsFactors = F
...
if (all(fieldDataSheet$Response == 1) | all(fieldDataSheet$Response == 0)) {
  stop(
    "Please ensure that the Field Data includes both presence and absence (or pseudo-absence) data before continuing."
  )
}
...
if (is.na(tuningSheet$Parameter1) | is.na(tuningSheet$Parameter1Values)) {
  stop(
    "No tuning parameters defined. Please select at least one parameter and provide tuning values before continuing."
  )
}
...
if (!is.na(tuningSheet$Parameter2)) {
  ...
  parameterNames <- names(combos) <- c(
    tuningSheet$Parameter1,
    tuningSheet$Parameter2
  )
} else {
...
if (nrow(modelSheet) < 1) {
  nadf <- as.data.frame(matrix(
    ncol = ncol(modelSheet),
    nrow = 1,
    data = rep(NA, ncol(modelSheet))
  ))
...
if (modType == "brt") {
  ...
  if (is.na(modelSheet$LearningRate)) {
    modelSheet$LearningRate <- 0.001
  }
  if (is.na(modelSheet$BagFraction)) {
    modelSheet$BagFraction <- 0.75
  }
  if (is.na(modelSheet$MaximumTrees)) {
    modelSheet$MaximumTrees <- 10000
  }
  if (is.na(modelSheet$NumberOfTrees)) {
    modelSheet$NumberOfTrees <- 50
  }
  if (is.na(modelSheet$TreeComplexity)) {
    modelSheet$TreeComplexity <- 1
  }
}
...
siteDataWide <- siteDataWide[
  complete.cases(subset(
    siteDataWide,
    select = c(-UseInModelEvaluation, -ModelSelectionSplit, -Weight)
  )),
]
compCases <- nrow(siteDataWide)
if (compCases / allCases < 0.9) {
  updateRunLog(paste(
    "\nWarning: ",
    round((1 - compCases / allCases) * 100, digits = 2),
    "% of cases were removed because of missing values.\n",
    sep = ""
  ))
}

# set site weights to default of 1 if not already supplied
if (all(is.na(siteDataWide$Weight))) {
  siteDataWide$Weight <- 1
}
...
factorInputVars <- covariatesSheet$CovariateName[which(
  covariatesSheet$IsCategorical == T &
    covariatesSheet$CovariateName %in% names(siteDataWide)
)]
...
trainTestDatasets <- split(
  siteDataWide,
  f = siteDataWide[, "UseInModelEvaluation"],
  drop = T
)
rm(siteDataWide)
gc()
...
if (nrow(trainingData) / (length(out$inputVars) - 1) < 10) {
  updateRunLog(paste(
    ...
    round(nrow(trainingData) / (ncol(trainingData) - 1), digits = 1),
...
p1Name <- paste0(
  modelArgs[parameterNames[1]],
  ": ",
  comboImgs[[parameterNames[1]]]
)
if (length(parameterNames) > 1) {
  p2Name <- paste0(
    modelArgs[parameterNames[2]],
    ": ",
    comboImgs[[parameterNames[2]]]
  )
...
for (r in 1:nrow(combos)) {
  # loop fits a model for each parameter combo
  ...
  capture.output(
    cat(
      as.character(tuningModelSheet$Model),
      " Results [",
      comboImgs$displayName[r],
      "]"
    ),
  ...
  if (is.null(finalMod)) {
    comboImgs$ModelsID[r] <- modelsSheet$ModelName[
      modelsSheet$ModelType == modType
    ]
    comboImgs[
      r,
      c(
    ...
    comboImgs$TextOutput[r] <- file.path(
      out$tempDir,
      paste0(modType, "_output.txt")
    )
  } else {
    ...
    if (modType == "rf") {
      ...
      txt0 <- paste(
        ...
        out$modOptions$NumberOfVariablesSampled,
        if (out$pseudoAbs == TRUE) {
          "\n\t   (averaged over each used available split)\n"
        },
        "\n\tn trees                                : ",
        out$modOptions$NumberOfTrees,
    ...
    if (modType == "brt") {
      ...
      txt0 <- paste(
        ...
        "\n\tn(trees)                     : ",
        nTrees,
        "\n\tn folds (used in final model fit) : ",
        cvFolds,
    ...
    saveRDS(
      finalMod,
      file = file.path(out$tempDir, paste0(modType, "_model.rds"))
    )
    ...
    if (cvFailed) {
      ...
      comboImgs$ModelRDS[r] <- file.path(
        out$tempDir,
        paste0(modType, "_model.rds")
      )
      comboImgs$ResponseCurves[r] <- file.path(
        out$tempDir,
        paste0(modType, "_ResponseCurves.png")
      )
      comboImgs$TextOutput[r] <- file.path(
        out$tempDir,
        paste0(modType, "_output.txt")
      )
      comboImgs$ResidualSmoothPlot[r] <- file.path(
        out$tempDir,
        paste0(modType, "_ResidualSmoothPlot.png")
      )
      comboImgs$ResidualSmoothRDS[r] <- file.path(
        out$tempDir,
        paste0(modType, "_ResidualSmoothFunction.rds")
      )
      comboImgs$ConfusionMatrix[r] <- file.path(
        out$tempDir,
        paste0(modType, "_ConfusionMatrix.png")
      )
      comboImgs$VariableImportancePlot[r] <- file.path(
        out$tempDir,
        paste0(modType, "_VariableImportance.png")
      )
      comboImgs$VariableImportanceData[r] <- file.path(
        out$tempDir,
        paste0(modType, "_VariableImportance.csv")
      )
      comboImgs$ROCAUCPlot[r] <- file.path(
        out$tempDir,
        paste0(modType, "_ROCAUCPlot.png")
      )
      comboImgs$CalibrationPlot[r] <- file.path(
        out$tempDir,
        paste0(modType, "_CalibrationPlot.png")
      )

      if (paste0(modType, "_StandardResidualPlots.png") %in% tempFiles) {
        comboImgs$ResidualsPlot[r] <- file.path(
          out$tempDir,
          paste0(modType, "_StandardResidualPlots.png")
        )
      }
      if (paste0(modType, "_AUCPRPlot.png") %in% tempFiles) {
        comboImgs$AUCPRPlot[r] <- file.path(
          out$tempDir,
...
combineTxtFiles(
  filePaths = txtFilePaths,
  outputPath = file.path(ssimTempDir, "OutputSummary.txt")
)

# Shiny App --------------------------------------------------------------------

launchShinyApp(
  file.path(packageDir, "07-hyperparameter-tuning-app.R"),
  appName = "Hyperparameter Tuning App"
)
...
if (all(comboImgs$fitFailed)) {
  updateRunLog(
    "No successful model fits to save. Please adjust tuning parameters and try again."
  )
} else {
  # update model datasheet
  modelSheet <- outAll[[selectedComboOutputs$displayName]]$modOptions %>%
    dplyr::select(-thresholdOptimization)
  if (modType == "rf") {
    saveDatasheet(myScenario, modelSheet, "wisdm_RF")
  }
  if (modType == "brt") {
    saveDatasheet(myScenario, modelSheet, "wisdm_BRT")
  }

  # save model outputs for selected model
  saveDatasheet(
    myScenario,
    selectedComboOutputs[, names(outputModelSheet)],
    "wisdm_OutputModel",
    append = T
  )
}
...
outputHyperparameterTuningSheet$ConfusionMatrix <- file.path(
  ssimTempDir,
  "ConfusionMatrixMatrix.png"
)
outputHyperparameterTuningSheet$VariableImportancePlot <- file.path(
  ssimTempDir,
  "VariableImportancePlotMatrix.png"
)
outputHyperparameterTuningSheet$ROCAUCPlot <- file.path(
  ssimTempDir,
  "ROCAUCPlotMatrix.png"
)
outputHyperparameterTuningSheet$CalibrationPlot <- file.path(
  ssimTempDir,
  "CalibrationPlotMatrix.png"
)
outputHyperparameterTuningSheet$TextOutput <- file.path(
  ssimTempDir,
  "OutputSummary.txt"
)
if ("StandardResidualPlotsMatrix.png" %in% tempFiles) {
  outputHyperparameterTuningSheet$ResidualsPlot <- file.path(
    ssimTempDir,
    "StandardResidualPlotsMatrix.png"
  )
}
if ("AUCPRPlotMatrix.png" %in% tempFiles) {
  outputHyperparameterTuningSheet$AUCPRPlot <- file.path(
    ssimTempDir,
    "AUCPRPlotMatrix.png"
  )
}
